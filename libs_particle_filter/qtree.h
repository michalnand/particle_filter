...
template <class t_data> class QTree
{
  public:
    ...
    void insert(t_data data, int x, int y)
    {
      if (y > m_y)
      {
        if (x > m_x)
    ...
    protected:
      ...
      void put_to(unsigned int idx, t_data &data, int x, int y)
      {
      ...
      t_data* get_from(unsigned int idx, int x, int y)
      {
        if (m_next[idx] != nullptr)
